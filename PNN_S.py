# -*- coding: utf-8 -*-
"""
Created on Sun Jun 18 13:25:56 2023

@author: rz
based on
https://www.analyticsvidhya.com/blog/2023/04/bayesian-networks-probabilistic-neural-network-pnn/
adopted by rz
...
#from sklearan.model_selection import StratifiedKFold
...
CVN = 5
skfold = model.StratifiedKFold(n_splits=CVN)
PK_vec = np.zeros(CVN)
...
for kernels_ind in range(len(kernels_vec)):
  ...
  for sigmas_ind in range(len(sigmas_vec)):
    ...
    print("Kernel = {}, sigma = {:2}:, PK {:<2}:".format(kernels_vec[kernels_ind], sigmas_vec[sigmas_ind], PK_krn_sig_vec[kernels_ind, sigmas_ind]))
